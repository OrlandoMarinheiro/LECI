...
{
    uint32_t id;
    uint32_t ni;   // number of items to produce
};
...
int main (int argc, char *argv[])
{
    ...
    // pthread_kill(cthr[1], SIGKILL); Training ex 1 - Question 1
    /* wait for consumers to finish */


    // training ex 2 - Question 1
    for (uint32_t i = 0; i < nc; i++)
    {
        Item terminationItem = {0, 0, 0};   
        fifoInsert(theFifo, terminationItem);
    }
