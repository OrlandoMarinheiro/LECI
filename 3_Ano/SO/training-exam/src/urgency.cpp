...
void init_simulation(int np)
{
   ...
   //inicializar mutux e cond para cada paciente
   for (int i = 0; i < npatients; i++)
   {
      cond_init(&hd->all_patients[i].is_done,NULL);
      mutex_init(&hd->all_patients[i].access_mutex, NULL);  
   }
...
void term_simulation(int np) {
   ...
   //apagar mutux e cond para cada paciente
   for (int i = 0; i < npatients; i++)
   {
      cond_destroy(&hd->all_patients[i].is_done);
      mutex_destroy(&hd->all_patients[i].access_mutex);
   }
...
int doctor_iteration(int id) // return value can be used to request termination
{
   ...
   mutex_lock(&hd->all_patients[patient].access_mutex);

   hd->all_patients[patient].done = 1;

   cond_broadcast(&hd->all_patients[patient].is_done);
   mutex_unlock(&hd->all_patients[patient].access_mutex);
...
void patient_goto_urgency(int id)
{
   ...
   printf("\e[36;01mPatient %s (number %d): get to hospital\e[0m\n", hd->all_patients[id].name, id);
...
void patient_wait_end_of_consultation(int id)
{
   ...
   mutex_lock(&hd->all_patients[id].access_mutex);

   while (!hd->all_patients[id].done){
      cond_wait(&hd->all_patients[id].is_done, &hd->all_patients[id].access_mutex);
   }
   printf("\e[36;01mPatient %s (number %d): health problems treated\e[0m\n", hd->all_patients[id].name, id);

   mutex_unlock(&hd->all_patients[id].access_mutex);
}

// TODO point: changes are required to this function
int patient_life(int id)
{
...
void* nurse(void* arg){
   ...
   // enquanto a nurse retornar 0 pode examinar outro paciente
   while (!nurse_iteration(id));
...
void* doctor(void* arg) {
   ...
   // enquanto o doctor retornar 0 pode receber outro paciente
   while(!doctor_iteration(id));
...
int main(int argc, char *argv[])
{
   ...
   for (int i = 0; i < npatients; i++){ 
      pData[i] = i;
      thread_create(&allpatients[i], NULL, patient, &pData[i]);
   ...
   for (int i = 0; i < nnurses; i++){
      nData[i] = i;
      thread_create(&nurses[i], NULL, nurse, &nData[i]);
   }
   ...
   for (int i = 0; i < ndoctors; i++){  
      dData[i] = i;
      thread_create(&doctors[i], NULL, doctor, &dData[i]);
   }
