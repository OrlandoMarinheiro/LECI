...
stat:
    (assignment | expr)? NEWLINE ;  // Uma expressão ou uma atribuicao opcional seguida de uma nova linha (NEWLINE)
                                    // O "?" significa que "expr" é opcional e a linha pode ter ou não uma expressão antes da nova linha.

assignment : ID '=' expr;           // estrutura para reconhecer atribuição de valores a variáveis (x = 3)
