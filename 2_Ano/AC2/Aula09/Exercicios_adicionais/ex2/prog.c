...
int setPWM(unsigned int dutyCycle) {
...
void configureAll(){
    ...
    TRISB = (TRISB & 0x80FF) | 0x0003; // Configurar RB8 a RB14 como saída
...
int main(void) {
    ...
    while(1){
        if(PORTBbits.RB0 == 1 && PORTBbits.RB1 == 0) {
            set_OC1R = setPWM(100);
            T1CONbits.TON = 0;
            IEC1bits.AD1IE = 0; // enable A/D interrupts
        } else if (PORTBbits.RB0 == 0 && PORTBbits.RB1 == 0){
            set_OC1R = setPWM(0);
            T1CONbits.TON = 1;
            IEC1bits.AD1IE = 1; // enable A/D interrupts
        } else {
            set_OC1R = setPWM(voltage);
            T1CONbits.TON = 1;
            IEC1bits.AD1IE = 1; // enable A/D interrupts
        }
